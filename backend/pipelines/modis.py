...
def _qc_ok(qc_val: int) -> bool:
    ...
    # bits 0-1: Mandatory QA flags: 00 good, 01 other quality, 10 TBD, 11 not produced
    # bits 2-3: Data quality: 00 good, 01 other quality, 10 TBD, 11 not useful
    mandatory = qc_val & 0b11
    data_quality = (qc_val >> 2) & 0b11
    return (mandatory == 0) and (data_quality == 0)
...
async def process_modis_for_land_day(land_id, date: str, collection_concept_id: str = None) -> dict:
    ...
    # Search backward until we find a day that actually yields usable LST.
